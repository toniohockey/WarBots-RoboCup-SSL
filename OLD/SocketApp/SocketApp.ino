...
const double constFL = 0.9;
const double constFR = 0.9;
const double constBK = 0.9;
...
void pwmProcess(){
  ...
  for( int l = 0 ; l < 2 ; l++ ){
    for( int i = 0 ; i < 8 ; i++ ){
      myLev = 0;
      if (srPins[i+8*l] > myPos)
        myLev = 1;
      bitWrite(currVal[l],i,myLev );
    }
    srvals[l] = currVal[l];
  }
...
// bit operations seems to be faster than digital write
  ...
  shiftOut(dataPin, clockPin, MSBFIRST, srvals[1]);
  shiftOut(dataPin, clockPin, MSBFIRST, srvals[0]);
...
{
    int motor[3] = {motor1, motor2, motor3};
    int m1 = (double)motor1 * constFL;
    int m2 = (double)motor2 * constFR;
    int m3 = (double)motor3 * constBK;
...
void stopMovement(){
   ...
   delay(100);  //might want to replace this delay
   setMotors(0, 0, 0);
}
